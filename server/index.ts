...
import { createCockpit } from './cockpit-session.ts';
...
app.post('/api/start', async (_req, res) => {
  ...
  try {
    game.start(); game.awaitFleetLaunch();
    const evidence = cockpit.begin(audit); starting = true;
...
app.post('/api/reset', (_req, res) => {
  try { if (starting || stopping) throw new Error('Wait for the current start/stop to finish'); game.reset(); cockpit.reset(); res.json(game.state); }
