...
test('automatic jamming partitions native Zenoh while cameras continue and queued team traffic resumes exactly once', { timeout: 45_000 }, async t => {
  ...
  const session = new TeamSession({ projectDir, game, onStatus: () => {}, onNetwork: state => states.push(state),
    ...
    vehicle: () => ({ start: async () => {}, stop: async () => {}, command: async (_id, args) => args,
      sample: async (_id, pose, simTime) => ({ position: { x: pose.x, y: pose.y, z: pose.z }, heading: { degrees: (360 - pose.yaw) % 360 }, simTime }) }),
  });
  t.after(async () => {
  ...
  const jam = body(await game.tool('drone-1', 'jam', { mission: 1, enabled: true }));
  ...
  await session.reconcileRadio();
  await eventually(() => delivered(outgoing.queued).length === 2 && delivered(incoming.queued).length === 1
  ...
  await delay(400);
  assert.deepEqual(delivered(outgoing.queued).map(event => event.drone).sort(), ['drone-2', 'drone-3']);
