...
async function until(game: FleetGame, check: () => boolean, timeout = 5000): Promise<void> {
  const expires = performance.now() + timeout;
  while (performance.now() < expires) {
    if (check()) return;
    const job = game.state.drones[0].job;
    assert.ok(!job || !['failed', 'cancelled', 'completed'].includes(job.state), `Routine ended before the expected state: ${JSON.stringify(job)}`);
    await delay(10);
  }
  assert.fail(`Onboard fixture deadline: ${JSON.stringify(game.state.drones[0].job)}`);
}
...
test('game workspace and real routine tools save only own telemetry with one camera per model boundary', async t => {
  ...
  await until(game, () => game.state.drones[0].job?.state === 'completed');
...
test('queued objectives preserve current execution until received; receipt cancels before later effects', async t => {
  ...
  await until(game, () => game.onboardWorkspace('drone-1').list().some(file => file.path === 'started.md'));
