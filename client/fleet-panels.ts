...
export class FleetPanels {
  ...
  private create(id: string): Panels {
    ...
    const card = document.createElement('article'); card.className = 'feed-card'; card.dataset.drone = id; card.dataset.droneId = id;
  ...
  update(state: WorldState, connectionReady: boolean) {
    ...
    for (const drone of state.drones) {
      ...
      put('.drone-connection', drone.online ? 'CONNECTED' : 'OFFLINE'); items.card.classList.toggle('drone-online', drone.online);
