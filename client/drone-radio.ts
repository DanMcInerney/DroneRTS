...
/** An outgoing radio transcript owned by one drone's camera card. */
export class DroneRadio {
  ...
  constructor(root: HTMLElement, private droneId: string) {
    ...
    this.log.setAttribute('aria-label', `${label} sent radio messages`);
  ...
  update(radio: RadioMessage[]) {
    const messages = radio.filter(message => message.from === this.droneId);
    ...
    const rows = messages.map(message => {
      ...
      const to = document.createElement('strong'); to.textContent = `→ ${message.to === 'all' ? 'TEAM' : dronePresentation(message.to).label.toUpperCase()}`;
      ...
      meta.append(to, kind, time); row.append(meta, body); return row;
    });
