...
export const layout = `
  ...
  <main>
    ...
    <section class="mechanics-strip" aria-label="How to play">
      <div class="mechanics-intro"><span class="eyebrow">THE RULES OF ENGAGEMENT</span><h2>Learn. Equip. Survive.</h2><p>Each team starts with ${RTS_CONFIG.startingCredits} shared salvage for one item. Discover yellow resource cubes and fly inside to mine. Colored service cubes automatically charge friendly drones and provide refits. Each drone fits two modules plus armor.</p></div>
      <div class="item-rule"><span class="item-icon" aria-hidden="true">↗</span><strong>Gun <b>${RTS_CONFIG.prices.gun} CR</b></strong><p>${RTS_CONFIG.magazineSize} rounds per magazine. Camera aim, gravity, cover and friendly fire matter. Rearm inside a friendly service cube for ${RTS_CONFIG.rearmCost} salvage and ${RTS_CONFIG.serviceDuration} seconds.</p></div>
      <div class="item-rule"><span class="item-icon" aria-hidden="true">⬡</span><strong>Armor <b>${RTS_CONFIG.prices.armor} CR</b></strong><p>Every drone starts with one free armor charge. It absorbs one bullet or collision, then breaks and alerts the drone. Replacement armor costs ${RTS_CONFIG.prices.armor} salvage. Empty batteries still cause a fatal power-loss crash.</p></div>
      <div class="item-rule"><span class="item-icon" aria-hidden="true">◆</span><strong>Mining drill <b>${RTS_CONFIG.prices.miner} CR</b></strong><p>Gather salvage ${RTS_CONFIG.minerRate / RTS_CONFIG.miningRate}× faster into the shared wallet. Resource cubes have room for all three drones and disappear when exhausted. Upgrade for ${RTS_CONFIG.prices.miner_upgrade} salvage. The center holds a richer deposit.</p></div>
      ...
      <div class="item-rule"><span class="item-icon" aria-hidden="true">▰</span><strong>Battery pack <b>${RTS_CONFIG.prices.battery} CR</b></strong><p>Double capacity from ${RTS_CONFIG.batteryCapacity} to ${RTS_CONFIG.extendedBatteryCapacity}. Friendly service cubes refill charge automatically while you're inside. Moving, mining and jamming use extra power. Empty charge is fatal.</p></div>
      <div class="item-rule"><span class="item-icon" aria-hidden="true">⌁</span><strong>Radio jammer <b>${RTS_CONFIG.prices.jammer} CR</b></strong><p>Spend power to interrupt nearby team radio, including your own and allies'. Cameras and flight control continue. Messages queue until reconnection or expiry.</p></div>
    </section>
    </details>
    <details class="network-lab panel"><summary class="network-heading"><div><span class="eyebrow">NETWORK LAB</span><h2>Inspect the peer mesh</h2><p>Native Zenoh peers and MAVLink packets. Isolate a drone to interrupt its team radio.</p></div><span id="network-status">Launch the match to connect.</span></summary><div class="network-peers" id="network-peers"></div><p class="network-note">An isolated drone keeps sensing and following its last received mission. Radio traffic queues until reconnection or expiry. These loopback connections simulate partitions, not RF propagation.</p></details>
      <section class="overview-panel panel" aria-label="Live overhead map">
        ...
        <div class="overview-body"><div class="overview-map" id="overview-map" role="button" tabindex="0" aria-label="Enter god view at this map location"><span class="map-north" aria-hidden="true">↑<br>N</span><div class="map-legend"><span class="legend-blue">● Blue</span><span class="legend-red">● Red</span><span class="legend-resource">◆ Salvage</span></div><span class="map-enter-hint">Click anywhere for God view <span>↗</span></span><div class="match-result" id="match-result" hidden role="status"><span class="eyebrow">MATCH COMPLETE</span><h2 id="result-title"></h2><p id="result-description"></p><button class="button button-primary" id="play-again">New match <span>↗</span></button></div></div>
          <aside class="map-sidebar"><div><span class="eyebrow">FINITE RESOURCES · SHARED INCOME</span><h3>Control the salvage.</h3><p>Yellow cubes hold salvage. Blue and red service cubes recharge their own team. Fly inside the volume to use it.</p></div><div class="resource-list" id="resource-list"></div><div class="map-fleet-list" id="map-fleet-list"></div><div class="map-explore-help"><strong>Go inside the city.</strong><p>Explore with <kbd>WASD</kbd>, mouse look, and <kbd>Shift</kbd> for speed.</p><span><kbd>Q / E</kbd> Down / up &nbsp; <kbd>Esc</kbd> Exit</span></div></aside></div>
