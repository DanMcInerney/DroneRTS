...
export class ReplayPlot {
  ...
  draw(model: ReplayTimeline, sample: ReplaySample, selected: DroneId | undefined, extent: ReplayExtent) {
    ...
    for (const resource of sample.frame.match?.resources ?? []) {
      const rich = cargoRules ? resource.capacity >= 600 : (resource.extractionMultiplier ?? 1) > 1, size = rich ? 5 : 3;
      if (!cargoRules && resource.zoneSize !== undefined && resource.remaining <= 0) continue;
      const at = xy(resource);
      if (resource.zoneSize !== undefined) {
        const side = resourceZoneSize(resource) * scale;
        ctx.fillStyle = resource.remaining > 0 ? '#ffe05c22' : '#555d5922'; ctx.strokeStyle = resource.remaining > 0 ? '#ffe05c' : '#7d8175'; ctx.lineWidth = 1;
      ...
      if (rich) { ctx.font = '9px monospace'; ctx.textAlign = 'center'; ctx.fillText(`MEGA · ${Math.ceil(resource.remaining)}${cargoRules ? '' : ` · ${resource.extractionMultiplier}×`}`, at.x, at.y + 18); }
    }
    ...
    for (const drone of drones) {
      ...
      const signal = drone.alive === false ? ' ×' : `${drone.cargo?.amount ? ` · ◆${drone.cargo.amount}` : ''}${drone.jamming ? ' · JAM' : drone.radioJammed ? ' · RF' : ''}`;
