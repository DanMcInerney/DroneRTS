...
| Native actor hosting | `server/runtime*.ts` | Codex model verification, isolated settings, fixed clean child bootstrap, role bearer tokens/policy and dynamic MCP catalogs. |
...
Objectives take effect only when received by that drone through the team's native network. Ordinary player/peer chat is independent of objectives. Radio partitions preserve valid local work and the last received mission. Stop is host cleanup, not a message crossing a broken radio. Death/Stop/reset, invalid capabilities, stale required sensing, lease failure and received objective changes invalidate affected work. Collision absorption stops movement. A model's private reasoning cannot be interrupted to insert input; runtime catalog refresh waits for a supported completed-turn boundary.
