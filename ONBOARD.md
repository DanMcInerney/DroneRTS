...
**Current native path:** [Nervelet](NERVELET-INTEGRATION.md) wraps these boundaries. Echo `nervelet.id` in `seen`; effects also use `nervelet.nextCommandId` as `command_id`. Included events remain unread until that acknowledgement, with stable IDs for uncertain redelivery. Received goals repeat exactly; native recovery gates new effects until an acknowledged recovery bundle. Conditional `step` waits are bounded to 30 seconds. The direct `FleetGame.tool` fixture/SDK seam retains its original consumption behavior. This supersedes the older assembly-time consumption description below for native pilots.
