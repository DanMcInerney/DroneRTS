...
Encoded output has a separate bounded transient budget. A 64 KiB file containing 65,536 NUL characters expands to 393,216 bytes of JSON content before framing, so the 400 KiB command-input ceiling and maximum-size file support remain. Read-result storage and escaped delivery are reserved before dispatch; several large outstanding results can reject new admission with explicit backpressure until `seen` releases capacity. No new mutation executes if its result reservation cannot be guaranteed. [Reliability QA](https://github.com/DanMcInerney/DroneRTS/blob/72717d725166f5e4dca1870c55640152ed9e0d49/RELIABILITY-QA.md) records maximum-result, aggregate-pressure and metadata evidence; fixed application partitions are unchanged.
...
Deterministic tests exercise quotas, Unicode byte counts, atomic failure, source retention, import identity, runaway code, slow host calls, stale jobs, partitions and real protocol storage. [RTS-PLAYTEST.md](https://github.com/DanMcInerney/DroneRTS/blob/72717d725166f5e4dca1870c55640152ed9e0d49/RTS-PLAYTEST.md) records autonomous evidence and its tested source revisions. Camera and autonomy reports must record current source hashes separately. A working neutral routine or fixture haul is not proof that a fresh model can recognize a cache, deliver cargo or coordinate. Performance targets in the [execution refinement](https://github.com/DanMcInerney/DroneRTS/blob/72717d725166f5e4dca1870c55640152ed9e0d49/ONBOARD-FEEDBACK-REFINEMENT.md) remain measurement targets until current timing evidence establishes them.
