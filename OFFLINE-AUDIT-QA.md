...
Pre-review verification evidence was `artifacts/test-runs/2026-09-16T19-21-10-796Z-offline-audit-final-c389f969/`, containing the suite/build/focused logs and the two stopped native-test network stores. No Node/Python process with this checkout in its command line remained at cleanup inspection. The full default-concurrency failure and its source interpretation are recorded above before its managed raw run was pruned. The original battle remains subject to its own checkout's retention policy.
...
Rebased onto `2a7eded` (the current public-documentation/CI baseline), retaining its README and adding the audit command. The combined revision passed **563/563 local tests** at concurrency two and the production build. One independent review identified four gaps: a missing terminal frame could skip summary comparison; exchange outcomes could hide nested failures; generic error deduplication could discard every failure; and the opening check missed completed purchases/shots and individual exchange effects.

The repair stays in the existing decoder and checks. Required sampled coverage now includes a terminal frame. Available summary fields are compared independently of that frame. Nested exchange errors retain receipt and operation identity, generic failures survive without invented details, and the opening check distinguishes successful individual operations from rejected/launch-pending outcomes. New regressions reproduce all four findings, including aggregate CLI exit statuses. The conservation fixture now gives its end marker the time of its actual final frame.

After repairs, **35/35 focused analysis tests** and the production build passed. Two evaluations of the retained battle produced report ID **`b8b83f61acdfb05c991b778b69136dc03ffda88e456f34e279e00874f61e2e3f`**, analyzer digest **`ab22eb1c76931ff6e3067cca9de1accc9b5a03cec38d8258765dc21edef6957a`**, and the same 535 inputs, 15 passing required checks, two optional gaps and six error incidents described above. Analyzer identity hashes source bytes, including checkout line endings.

The initial PR CI run (`35142889181`, before review repairs) passed Linux. Windows passed all audit cases but finished **560/563**: `onboard-game` did not reach its expected state before its bounded deadline; the slow-host callback and six-worker routine cases encountered `guest_slice_deadline`. The repair changes no runtime budget or assertion in those tests. The repaired revision receives a fresh full CI run before merge.

Newest local evidence at this stage is `artifacts/test-runs/2026-09-16T19-48-06-868Z-tests-ad84a5a7/`, with `tests.log`, `build.log`, `review-regressions.log` and its two stopped native-test network stores. Earlier managed evidence was pruned normally after analysis. Retrying deletion of the four older residual network stores after explicit user authorization was again rejected with `blocked by policy`; they remain ignored local artifacts.
