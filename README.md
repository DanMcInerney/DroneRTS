...
Click a drone card or its **Cockpit** link to inspect that actor's camera image, sensor bundle, unread message batch, tool requests/results and outgoing messages. The bottom feed follows emitted output, tool activity, **Native reasoning** and **Reasoning summary** entries when the runtime supplies readable text. Its availability indicator distinguishes waiting, native text, summaries, both or no readable text. Cockpit images come from actual MCP responses, with acquisition and response timestamps; they do not advance with the live FPV view. Response recording does not acknowledge when the model read it. An empty inbox batch is shown as empty. The workspace and compute cards describe the actor's real permissions: fleet MCP tools only, with no shell, writable files, script execution or direct library imports. Host adapters use Zenoh and pymavlink. See [COCKPIT.md](COCKPIT.md) for the evidence contract and extension points.
...
All drones begin unequipped with zero team credits. Exactly three fixed cargo caches deplete as drones mine nearby: one halfway from each team's base to the battlefield center, and an extra-large cache at the center. Each side cache holds 120 credits; the center holds 480. Their physical cargo crates, salvage labels and landing-pad markings appear in the actual camera. Income immediately enters the team's shared bank; there is no hauling or base construction. Gathering requires a recent camera observation and physical reach, with no added landing prerequisite. An equipped miner gathers three times as fast as a bare drone. Buying is local to the purchasing drone, so teammates must agree who spends the shared bank on which role.
...
The compact actor prompt lives in `server/runtime-tools.ts`; [DRONE-PROMPT.md](DRONE-PROMPT.md) contains the rendered text and compute constraints. `shared/actor-environment.ts` centralizes the model, reasoning effort and permitted environment facts used by the prompt and cockpit.
