...
| `send({mission,to,kind,text,data?})` | Team radio; blue alone may address `player`. Short-lived `status` is distinct from durable chat. |
...
[ARCHITECTURE.md](ARCHITECTURE.md) maps ownership. [CITY.md](CITY.md) describes private battlefield layout and the separate vehicle calibration; [CINCINNATI-SOURCES.md](CINCINNATI-SOURCES.md) records geographic evidence and approximations. [NETWORK.md](NETWORK.md) and [MAVLINK.md](MAVLINK.md) describe the native protocols. [AGENTS.md](AGENTS.md) gives development constraints.
...
node --import tsx scripts/analyze-haul.ts artifacts/focused-trials/<run-directory>
