...
The next Windows run passed that quota fixture but its single 40 MiB memory-test allocation reached the CPU guard before reporting heap exhaustion. Splitting the probe into retained 1 MiB buffers exposed a pre-existing enforcement gap: all 40 MiB were retained despite the configured 32 MiB engine allocation limit. The worker now supplies private WebAssembly memory with a hard maximum of 32 MiB through the installed QuickJS binding's supported `newVariant` API, while retaining the engine limit. The ceiling includes interpreter overhead and ArrayBuffer backing storage. The regression yields between allocations, requires the exact `guest_heap_limit` result, and checks that the final write cannot occur. The subsequent CPU-runaway probe still checks termination after restarting the worker. No production budget was increased.

With both fixes applied, the full local Windows suite passed **565/565 tests**, with no failures, cancellations or skips, and the production build passed. The regenerated manifest measures **3,605,742 / 8,388,608 bytes** across 696 artifacts. Hosted results for this final revision are recorded on [PR #28](https://github.com/DanMcInerney/DroneRTS/pull/28).
